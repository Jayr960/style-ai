...
// --- Single Flat Lay Display ---
function OutfitGeneratedImage({ outfit, items }: { outfit: any; items: any[] }) {
  const existingGenerated = outfit.items?.find?.((i: any) => i.id === "generated_image");
  const [imageUrl, setImageUrl] = useState<string | null>(existingGenerated?.url || null);
  const [generating, setGenerating] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (imageUrl || generating || existingGenerated) return;

    const generate = async () => {
      setGenerating(true);
      try {
        const desc = items.map(w => `${w.color || ''} ${w.pattern || ''} ${w.item_type || w.tags?.join(' ') || 'item'}`).join(", ");
        const res = await supabase.functions.invoke("generate-outfit-image", {
          body: { outfitId: outfit.id, itemsDescription: desc, styleVibe: "fashion catalog flat-lay" }
        });
    ...
    generate();
  }, [outfit.id, imageUrl, items, generating, existingGenerated]);

  if (error) {
    return (
       <div className="flex p-8 aspect-video w-full flex-col items-center justify-center bg-secondary/30 gap-2 text-center rounded-xl my-4 mx-4 w-[calc(100%-2rem)]">
  ...
  if (!imageUrl) {
    return (
       <div className="relative p-8 aspect-video w-[calc(100%-2rem)] mx-4 my-4 flex items-center justify-center bg-secondary/10 overflow-hidden rounded-xl border border-border/50">
         <div className="absolute inset-0 z-0 bg-gradient-to-r from-transparent via-white/20 to-transparent skew-x-[-20deg] translate-x-[-200%] animate-[shimmer-sweep_2s_infinite]" />
         <div className="flex flex-col items-center gap-3 z-10">
           <Sparkles className="h-6 w-6 text-primary animate-pulse" />
           <p className="text-xs text-muted-foreground font-medium animate-pulse">Designing beautiful flat-lay...</p>
         </div>
       </div>
    );
  }
  ...
  return (
    <div className="relative mx-4 my-4 aspect-square md:aspect-video w-[calc(100%-2rem)] overflow-hidden rounded-xl border border-border/50 bg-white shadow-sm flex items-center justify-center">
      <img src={imageUrl} alt={outfit.outfit_name || "Outfit flat lay"} className="w-full h-full object-cover" />
    </div>
  );
}
...
const Outfits = () => {
  ...
  const getOutfitItems = (outfit: SavedOutfit) => {
    ...
    return items
      .filter((item: any) => item.id !== "generated_image")
      .map((item: any) => {
                ...
                <motion.div key={outfit.id} variants={fadeUp} layout>
                  <GlassCard className="p-0 overflow-hidden">
                    <div className="relative z-10">
                      ...
                      {/* Single Elegant Flat Lay Image Generated by AI */}
                      {outfitItems.length > 0 ? (
                        <OutfitGeneratedImage outfit={outfit} items={outfitItems} />
                      ) : (
                        <div className="flex aspect-video w-full items-center justify-center bg-secondary/30 mx-4 my-4 rounded-xl w-[calc(100%-2rem)]">
